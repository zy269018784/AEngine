...
RHIApplicationTexture2DSperateImageSampler::RHIApplicationTexture2DSperateImageSampler()
    : RHIApplication()
{
...
void RHIApplicationTexture2DSperateImageSampler::Init()
{
    std::cout << "1" << std::endl;
    CreateVBO();
    CreateEBO();
...
void RHIApplicationTexture2DSperateImageSampler::CreateVBO()
{
...
void RHIApplicationTexture2DSperateImageSampler::CreateEBO()
{
    RHIEBO = pRHI->RHICreateBuffer(RHIBufferType::IndexBuffer, RHIBufferUsageFlag::IndexBuffer, sizeof(Index), Index);
}

void RHIApplicationTexture2DSperateImageSampler::CreateTexture()
{
    ...
    */
    ...
    RHITexture2D = pRHI->RHICreateTexture2D(RHIPixelFormat::PF_R8G8B8A8_UNORM,
        RHITextureUsageFlag::SampledTexture,
        RHIImageLayout::RHI_IMAGE_LAYOUT_UNDEFINED,
        1, texWidth, texHeight, pixels);
...
void RHIApplicationTexture2DSperateImageSampler::CreateSRB()
{
    SRB = pRHI->RHICreateShaderResourceBindings();
    SRB->SetBindings({
        RHIShaderResourceBinding::Texture(0, RHIShaderType::Fragment, RHITexture2D),
        RHIShaderResourceBinding::Sampler(1, RHIShaderType::Fragment, RHISampler_),
    });
    SRB->Create();
}

void RHIApplicationTexture2DSperateImageSampler::CreateVertexDescriptioin()
{
#if 1
    /*
        使用VBO1
...
void RHIApplicationTexture2DSperateImageSampler::CreateGraphicsPipeline()
{
#if 1
    auto vertShaderCode = ReadFile("Texture2DSperateImageSampler_vert.spv");
    auto fragShaderCode = ReadFile("Texture2DSperateImageSampler_frag.spv");
    // 创建Shader
...
void RHIApplicationTexture2DSperateImageSampler::Draw()
{
    float x = 0;
    float y = 0;
    float w = 0;
