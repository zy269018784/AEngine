...
RHIApplicationUBO::RHIApplicationUBO()
    : RHIApplication()
{
   
}

void RHIApplicationUBO::Init()
{
    CreateVBO();
    CreateEBO();
    CreateUBO();
...
void RHIApplicationUBO::CreateUBO()
{
    for (int i = 0; i < 4; i++)
    {
        RHIUBO[i] = pRHI->RHICreateBuffer(RHIBuffer::RHIBufferType::UniformBuffer, RHIBuffer::RHIBufferUsageFlag::Static, sizeof(UBOData[i]), UBOData[i]);
    }
}

void RHIApplicationUBO::CreateSRB()
{
...
void RHIApplicationUBO::CreateVertexDescriptioin()
{
#if 0
    /*
        使用VBO1
