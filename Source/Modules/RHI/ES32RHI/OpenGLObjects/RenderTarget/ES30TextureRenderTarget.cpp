...
ES30TextureRenderTarget::ES30TextureRenderTarget(std::vector<OpenGLTexture *> InColorAttachments, std::vector<OpenGLTexture *> InDepthAttachments)
    : OpenGLTextureRenderTarget()
{
    ...
    for (std::uint32_t Index = 0; Index < DepthStencilAttachments.size(); ++Index)
    {
        ...
        DepthStencilAttachments[Index] = new ES30Attachment(Type, InColorAttachments[Index]);
    }
}
