...
ES20TextureRenderTarget::ES20TextureRenderTarget(std::vector<OpenGLTexture *> InColorAttachments, std::vector<OpenGLTexture *> InDepthAttachments)
    : OpenGLTextureRenderTarget()
{
    ...
    for (std::uint32_t Index = 0; Index < ColorAttachments.size(); ++Index)
    {
        ...
        ColorAttachments[Index] = new ES20Attachment(Type, InColorAttachments[Index]);
    }
    ...
    for (std::uint32_t Index = 0; Index < DepthStencilAttachments.size(); ++Index)
    {
        ...
        DepthStencilAttachments[Index] = new ES20Attachment(Type, InDepthAttachments[Index]);
    }
}
