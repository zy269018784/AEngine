...
ES30Framebuffer::ES30Framebuffer(std::uint32_t Width, std::uint32_t Height,
    ...
    : ES30Framebuffer()
{
    ...
    for (int i = 0; i < InColorAttachments.size(); i++)
    {
        ...
        GLuint Texture;
        //dynamic_cast<OpenGLColorAttachment *>(InColorAttachments[i])->GetHandle();
...
GLuint ES30Framebuffer::GetHandle() const
{
    return Handle;
}
