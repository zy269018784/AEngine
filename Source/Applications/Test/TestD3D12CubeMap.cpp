...
namespace {
    ...
    ID3D12Resource* g_cubemap = nullptr;
...
// 创建立方体贴图数据（6个面，不同颜色）
void CreateCubemapData(std::vector<uint8_t>& data, int face) {
    ...
    for (UINT y = 0; y < CUBEMAP_SIZE; ++y) {
        for (UINT x = 0; x < CUBEMAP_SIZE; ++x) {
            ...
            // 根据不同的面设置不同的颜色，并添加渐变效果
            float u = (float)x / CUBEMAP_SIZE;
            float v = (float)y / CUBEMAP_SIZE;

            switch (face) {
            case 0: // 右面 (+X) - 红色渐变
                data[index + 0] = (UINT)(255 * u);
                data[index + 1] = (UINT)(255 * v);
                data[index + 2] = 0;
                break;
            case 1: // 左面 (-X) - 绿色渐变
                data[index + 0] = 0;
                data[index + 1] = (UINT)(255 * u);
                data[index + 2] = (UINT)(255 * v);
                break;
            case 2: // 上面 (+Y) - 蓝色渐变
                data[index + 0] = (UINT)(255 * u);
                data[index + 1] = 0;
...
bool CreateCubemapTexture() {
    ...
    textureDesc.DepthOrArraySize = 6;  // 立方体贴图有6个数组元素
...
static bool CreatePipeline() {
    ...
    const char* psSource = R"(
        ...
        float4 main(PSInput input) : SV_TARGET {
            // 使用法线作为方向采样立方体贴图
            return g_cubemap.Sample(g_sampler, input.normal);
        }
    )";
...
int TestD3D12Cubemap(int argc, char **argv) {
    ...
    GLFWwindow* window = glfwCreateWindow(WIDTH, HEIGHT, "D3D12 Cubemap - Rotating Cube", nullptr, nullptr);
    if (!window) {
    ...
    std::cout << "========================================" << std::endl;
    std::cout << "Cubemap Demo - Rotating Cube" << std::endl;
    std::cout << "Each face shows a different color gradient" << std::endl;
    std::cout << "========================================" << std::endl;
